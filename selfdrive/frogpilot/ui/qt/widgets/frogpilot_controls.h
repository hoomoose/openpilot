...
class FrogPilotButtonsToggleControl : public ParamControl {
  Q_OBJECT
public:
  FrogPilotButtonsToggleControl(const QString &param, const QString &title, const QString &desc, const QString &icon,
                                const std::vector<QString> &button_texts, const int minimum_button_width = 225)
  : ParamControl(param, title, desc, icon), key(param.toStdString()) {
    const QString style = R"(
      QPushButton {
        border-radius: 50px;
        font-size: 40px;
  ...
  void refresh() {
    ...
    bool visible = params.getBool(key);
