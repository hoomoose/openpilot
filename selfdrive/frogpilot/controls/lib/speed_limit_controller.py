...
class SpeedLimitController:
  ...
  def update_speed_limits(self):
    self.car_speed_limit = json.loads(self.params_memory.get("CarSpeedLimit") or '0')
    self.map_speed_limit = json.loads(self.params_memory.get("MapSpeedLimit") or '0')
    self.nav_speed_limit = json.loads(self.params_memory.get("NavSpeedLimit") or '0')
  ...
  def write_car_state(self):
    self.params_memory.put("CarSpeedLimit", json.dumps(self.car_speed_limit))

  def write_map_state(self):
    self.params_memory.put("MapSpeedLimit", json.dumps(self.map_speed_limit))

  def write_nav_state(self):
    self.params_memory.put("NavSpeedLimit", json.dumps(self.nav_speed_limit))
