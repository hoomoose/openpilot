...
class ConditionalExperimentalMode:
  ...
  def update_conditions(self, lead, modelData, road_curvature, stop_distance, t_follow, v_ego, v_lead):
    ...
    self.lead_slowing_down(lead, stop_distance, t_follow, v_ego, v_lead)
  ...
  def lead_slowing_down(self, lead, stop_distance, t_follow, v_ego, v_lead):
    if self.lead_detected:
      lead_close = lead.dRel <= v_lead * t_follow
      lead_far = lead.dRel >= CITY_SPEED_LIMIT
      ...
      self.lead_stopping = self.lead_slowing_down_mac.get_moving_average() >= PROBABILITY and not lead_far
    else:
